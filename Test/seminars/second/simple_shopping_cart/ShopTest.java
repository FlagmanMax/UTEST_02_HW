...
//        Cart cart = new Cart(shop);
        ...
        // Assert
        assertThat(cart.getTotalPrice()).isEqualTo(170 + 250 + 250);
...
//        Cart cart = new Cart(shop);
        ...
        // Act
        cart.addProductToCartByID(4); // 170 +
        cart.addProductToCartByID(4); // 170 +
...
//        Cart cart = new Cart(shop);
    ...
    @ParameterizedTest
    @ValueSource(ints = {1})
    void incorrectProductRemoveCausesException(int id) {
        // Arrange
//        Shop shop = new Shop(getStoreItems());
//        Cart cart = new Cart(shop);

        // Assert
        assertThatThrownBy(()-> cart.removeProductByID(id)).isInstanceOf(RuntimeException.class).hasMessage("В корзине не найден продукт с id: "+id);
    }
